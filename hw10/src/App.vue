<template>
    <div id="app">
        ...
        <Middle :posts="posts" :users="users" :userId="userId"/>
...
export default {
    ...
    beforeCreate() {
        ...
        const inRange = function (text, left, right) {
            return text && text.length >= left && text.length <= right;
        }

        this.$root.$on("onRegister", (login, name, password) => {
            const users = Object.values(this.users).filter(u => u.login === login);
            if (users.length !== 0) {
                this.$root.$emit("onRegisterValidationError", "User with this login already exists");
            } else if (!login || !login.match("^[a-z]+$") || !inRange(login, 3, 16)) {
                this.$root.$emit("onRegisterValidationError", "Login should be within [3, 16] " +
                        "latin lowercase letters");
            } else if (!name || !name.replace(/\s/g, '').length || !inRange(name, 1, 32)) {
                this.$root.$emit("onRegisterValidationError", "Name should be within [1, 32] characters");
            } else if (!password || !inRange(password, 4, 64)) {
                this.$root.$emit("onRegisterValidationError", "Password should be within [4, 64] characters");
            } else {
            ...
            if (this.userId) {
                if (!title || title.length < 5) {
                    ...
                    this.$root.$set(this.posts, id, {
                        id, title, text, userId: this.userId, comments: []
                    });
                }
            } else {
